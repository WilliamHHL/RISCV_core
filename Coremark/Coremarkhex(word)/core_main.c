...
iterate(void *pres)
{
    ...
    res->crcmatrix           = 0;
    res->crcstate            = 0;
    ee_printf("=== ITERATE START ===\n");  // 这个会很快出现
    for (i = 0; i < iterations; i++)
    { //  ee_printf("=== ITER %u: calling list(1) ===\n", i);
        crc      = core_bench_list(res, 1);
       // ee_printf("=== ITER %u: list(1) done ===\n", i);  // 这个会很慢
        res->crc = crcu16(crc, res->crc);
       ee_printf("iter=%u step1 crcu16: 0x%04x\n", i, res->crc); // <-- 新增

        crc      = core_bench_list(res, -1);
        res->crc = crcu16(crc, res->crc);
       ee_printf("iter=%u step2 crcu16: 0x%04x\n", i, res->crc); // <-- 新增
        if (i == 0)
            res->crclist = res->crc;
    }
    return NULL;
